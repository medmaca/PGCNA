...
parser.add_argument(
    "-g",
    "--geneFrac",
    help="Fraction of files a gene needs to be present in to be included in median corr matrix [1/3]",
    type=float,
    default=1 / 3.0,
)
parser.add_argument(
...
parser.add_argument(
    "--noLeidenalg",
    help="Don't try to run Leidenalg clustering, but complete everything else [False] -- Flag",
    action="store_true",
)
parser.add_argument(
    "--usePearson",
...
parser.add_argument(
    "--ignoreDuplicates",
    help="Ignore correlation duplicates when cutting top --edgePG genes [False] -- Flag, faster if set",
    action="store_true",
)
parser.add_argument(
    "--singleCorr",
    help="Output individual gene correlation files -- Warning this generates 1 file per gene in final correlation matrix. [False] -- Flag",
    action="store_true",
)
parser.add_argument(
    "--singleCorrL",
    help="Output individual gene correlation files -- limited to those in --singleCorrListF [False] -- Flag",
    action="store_true",
)
...
parser.add_argument(
    "--singleCorrListF",
    help="If --singleCorrL is set then create single correlation files for all those in this file (must be within --workFolder)",
    default="corrGenes.txt",
)
...
parser.add_argument(
    "--corrChunk",
    dest="corrChunk",
    help="Size of chunk (rows) to split correlation problem over [5000] -- Higher will speed up correlation calculation at cost of RAM",
    default=5000,
    type=float,
)
...
parser.add_argument(
    "--lClustListFold",
    dest="lClustListFold",
    help="Leidenalg Clusters module list folder [ClustersLists]",
...
parser.add_argument(
    "--randSeed",
    dest="randSeed",
    help="Set random seed so that each run with the same settings is the same [42].",
    default=42,
    type=int,
)
...
random.seed(args.randSeed)  # Set random to make results repeatable
###########################################################################################
